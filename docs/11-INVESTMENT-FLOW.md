...
────────────────────────────────────────────┼────────────────────────────────────────
 ...
 5  يحرّك المراحل: Contacted → InDiscussion   ◀───────────▶│  غرفة الصفقة تفتح
                    │                                     │
 6  يرد على الأسئلة ويلبّي طلبات المستندات ◀──────────────┤  يسأل · يطلب مستندات · يطلب هو كمان
                    ▼                                     │
 6.5  يقترح شروط (term sheet)          ◀─────────────────▶│  يوافق على الشروط
      كلا الطرفين وافق → Stage = Committed                │
                    ▼                                     │
 7  «Request Funds» ── FundingRequest(Open) ──────────────▶│  ← PaymentDue
                                              ◀── counter-offer ──┤  (اختياري) يقترح رقم مختلف
                                              ── يوافق/يرفض ──────▶│
                                                          ▼
 8                                              يدفع (sandbox) ── Transaction
                                                          │
                    ◀──────── Succeeded ──────────────────┘  ← Funded ✅ (أو PartiallyFunded لو قسط)
                    ▼                                     │
...
| 8 · الدفعة تمّت (كاملة) | — | ثابت | **−** | **+** |
| 8 · الدفعة تمّت (قسط) | — | ثابت | **−** جزئي | **+** جزئي، الحالة `PartiallyFunded` |
| 10 · refund | — | ثابت | — | **−** |

> لاحظ: **Committed مبيتحرّكش عند الدفع.** الالتزام موجود من ساعة الموافقة (أو من ساعة الاتفاق على شروط، لو فيها)؛ الدفع بيضيف Funded جنبه، مش بدله.
>
> **الالتزام ممكن يتسدّد على أكتر من دفعة.** المؤسّس يقدر يطلب جزء دلوقتي والباقي بعدين — العلاقة تفضل `PartiallyFunded` لحد ما مجموع الدفعات يوصل للرقم المتّفق عليه (شروط متّفق عليها لو فيها، وإلا رقم الالتزام الأصلي).
...
| **يتابع** | صاحب السؤال، **مرّة واحدة**، وبعد ما ييجي رد | نفس `questions` بـ `parentQuestionId` | `deal_question` → **محتاجك** |
| يسحب | صاحب السؤال | `POST .../withdraw` | — |

> **متابعة واحدة بس، ومستوى واحد.** "وده على مدار أد إيه؟" بعد إجابة على "إيه معدّل الحرق؟" هي بالظبط الحالة اللي كانت بترجّع الطرفين للشات العادي — الـ thread هنا توضيح، مش منتدى.

### طلبات المستندات

| العملية | مين | الـ endpoint | الإشعار |
|---|---|---|---|
| يطلب مستند | **أي طرف** | `POST /api/deals/{id}/document-requests` | `doc_request` → **محتاجك** |
| يلبّي (اللي ماطلبش) | 🚀 بربط مستند من غرفة البيانات، 💰 برفع ملف/ملاحظة | `PUT /api/deals/document-requests/{rid}` | `doc_fulfilled` → نتيجة |
| يرفض بسبب | اللي ماطلبش | نفس الـ endpoint | `doc_declined` → نتيجة |
| يسحب | صاحب الطلب | `POST .../withdraw` | — |

> **الاتجاه اتفتح للطرفين.** كان المستثمر بس هو اللي يقدر يطلب — مؤسّس عمره ما كان يقدر يطلب إثبات ملاءة أو هوية ممّن بيموّله. **قوالب جاهزة** لكل اتجاه (جدول الملكية · القوائم المالية · العرض التقديمي · مستندات التأسيس ↔ إثبات ملاءة · إثبات هوية · تفويض استثمار · مراجع) توفّر كتابة العنوان من الصفر. **رد المستثمر بيتخزّن جوّه الطلب نفسه**، مش في غرفة البيانات العامة — عشان أي داعم معتمَد تاني بيقدر يشوف غرفة البيانات، وخطاب بنكي مش لكل الناس.
>
> **لو المؤسّس مسح المستند اللي كان بيلبّي الطلب** → الطلب **بيترجع مفتوح تلقائيًا** بدل ما يختفي معاه.

### الشروط المتّفق عليها (Term Sheet)

> 🔑 **دي اللي بتخلّي مرحلة `Committed` معناها حاجة حقيقية.** كانت موصوفة بـ"الطرفين اتفقوا على الشروط خارج المنصّة" — يعني أهم لحظة في العلاقة كانت بتحصل في مكان المنصّة مش شايفاه، ومسجّلة عندها كصندوق اختيار بس.

| العملية | مين | الـ endpoint |
|---|---|---|
| يقترح شروط | أي طرف — الاقتراح نفسه عدّ موافقة صاحبه | `POST /api/deals/{id}/terms` |
| يوافق | **الطرف التاني** | `POST /api/deals/terms/{sheetId}/accept` |
| يرفض بسبب | الطرف التاني | `POST /api/deals/terms/{sheetId}/decline` |

**بيتضمّن:** المبلغ · نسبة الأسهم (اختياري) · التقييم (اختياري) · استخدام الأموال · شروط تانية (مقاعد مجلس إدارة، دفعات، استحقاق — نصّ حرّ).

**الموافقتين متتبّعتين لوحدهم.** لما التانية توصل، الشروط تبقى `Accepted` والعلاقة تتحرّك لـ `Committed` **فعليًا**، مسجَّلة في الجدول الزمني برقم النسخة والمبلغ كسبب الحركة. اقتراح جديد وقت ما فيه واحد قائم بيستبدله (`Superseded`) بدل ما يترفض — عشان إعادة التفاوض متسيبش العلاقة من غير شروط خالص لحظة.

> **مش عقد.** فيستورا مبتحتفظش بتوقيعات ومبتنفّذش حاجة. ده سجلّ مكتوب لما الطرفين قالوا إنهم موافقين عليه.

**الجدول الزمني** بيجمّع كل حاجة بترتيب زمني: فتح العلاقة · **كل حركة مرحلة بمدّتها** · الأسئلة والأجوبة (بمتابعاتها) · طلبات المستندات (في الاتجاهين) · **الشروط المقترَحة والمتّفق عليها** · المستندات المنشورة · الرسائل · الرفض · إغلاق الجولة.

**مؤشّر صحة العلاقة** مشتقّ بيتعرض جنب المرحلة — `Healthy` / `Slowing` / `Stalled` — من السكون وعدد الأيام واللي لسه معلَّق (أسئلة من غير رد، مستندات من غير ردّ، فلوس مطلوبة ومدفوعتش، شروط مقترَحة ومحدّش وافق عليها). محدّش بيحدّثه يدويًا — وده اللي بيخلّيه يستاهل الثقة.

**محادثة الغرفة** — نفس الشات بتاع `/messages` مفلتر على المشروع ده، معروض جوّه `/deals/{id}` نفسها بدل ما المستخدم يفتح تاب تاني.
...
| 8 | الالتزام **متسدّدش بالكامل بعد** — مجموع الدفعات الناجحة ≥ الهدف |
| 9 | المبلغ > المتبقّي في الجولة (**بعد استبعاد التزام العلاقة دي نفسها، وبعد خصم أي جزء اتسدّد فعلًا منها**) |
| 10 | لو فيه شروط متّفق عليها، المبلغ ميتعدّاش الجزء اللي لسه متسدّدش منها |
...
>
> **الشرط 8 بقى بيقارن المجموع، مش "هل فيه دفعة نجحت من قبل".** الالتزام ممكن يتسدّد على أكتر من طلب (قسط دلوقتي، والباقي بعدين)؛ لو اتفحص بأول نجاح بس، أول قسط كان هيقفل الباب على الباقي للأبد ويسيب العلاقة `Committed` من غير طريقة تكمّل. طلب اترجّعت فلوسه بيفضل `Paid` عن قصد (هو **اتدفع** وبعدين **اتعكس**)، بس الفلوس راحت، فالمؤسّس لازم يقدر يطلب تاني.

**العرض المقابل (Counter-offer)** — بدل ما المستثمر يدفع رقم المؤسّس زي ما هو أو يسيبه معلَّق:

```
مستثمر يقترح رقم (POST /funding-requests/{id}/counter)
   ↓ محاولة دفع جارية على الطلب؟ اتلغت — رقمين مقترحين في نفس اللحظة تناقض
مؤسّس يوافق (POST .../counter/answer {accept:true})
   ↓
الطلب القديم يتقفل ("Superseded by the agreed …") + طلب جديد بالرقم المتفَق عليه
   ↓ (لو الطلب الجديد اترفض — سعة، جولة اتقفلت — الطلب القديم يرجع مفتوح تاني)
```

لو المؤسّس رفض العرض، الطلب الأصلي بيفضل مفتوح زي ما هو والمستثمر يقدر يدفعه أو يقترح رقم تاني.
...
> **لو ده مش أول قسط، ومجموع الدفعات لسه أقلّ من الهدف** — الحالة بتبقى `PartiallyFunded` مش `Funded`. المؤسّس يقدر يطلب الباقي بطلب تمويل جديد على نفس العلاقة. `Funded` بمعناها الكامل (تظهر في الحالة العامة للمشروع) بتيجي لما **مجموع** الدفعات يوصل للهدف — الهدف نفسه هو الشروط المتّفق عليها لو فيها، وإلا رقم الالتزام الأصلي.
...
| **الدفعة تمّت (كاملة)** | `Approved` | `Committed` | `Paid` | `Succeeded` | **Funded** ✅ |
| **الدفعة تمّت (قسط)** | `Approved` | `Committed` | `Paid` | `Succeeded` | **PartiallyFunded** — لسه فيه باقي |
...
| "أول دفعة ناجحة = العلاقة اتموّلت" | لو الالتزام على أقساط، لأ — الحالة `PartiallyFunded` لحد ما **مجموع** الدفعات يوصل الهدف. |
| "قبول عرض المستثمر المقابل بيعدّل رقم الطلب" | لأ — الطلب القديم بيتقفل ويتفتح طلب جديد بالرقم الجديد؛ المبالغ على صفّ مالي عمرها ما بتتكتب فوق بعضها. |
| "الشروط المتّفق عليها عقد" | لأ — سجلّ مكتوب بموافقة الطرفين. فيستورا مبتحتفظش بتوقيعات ومبتنفّذش حاجة. |
