// Phase 5: the founder requests the funds. This creates the funding request
// the investor is then asked to settle.
...
const dump = async (p, label) => {
  const r = await p.evaluate(() => ({
    ...
    dialogText: clean0(document.querySelector('[role="dialog"]')?.innerText || ''),
    body: clean0(document.body.innerText || '').slice(0, 900),
    controls: [...document.querySelectorAll('button, [role="button"]')]
      .map(e => ({ t: clean0(e.textContent), off: e.disabled === true }))
      .filter(o => o.t && o.t.length < 45).filter((v, i, s) => s.findIndex(x => x.t === v.t) === i),
    fields: [...document.querySelectorAll('input, textarea')]
      .map(e => ({ id: e.id || '', type: e.type || 'textarea', ph: e.placeholder || '', val: e.value || '' })),
  }));
  console.log(`\n===== ${label}  [${r.path}] dialog=${r.dialog}`);
  console.log(r.dialog ? 'DIALOG: ' + r.dialogText : r.body);
...
(async () => {
  ...
  await p.evaluateOnNewDocument(() => {
    window.clean0 = (s) => (s || '').replace(/\s+/g, ' ').trim();
  });
  ...
  await settle(p);
  console.log('clicked: ' + JSON.stringify(await clickText(p, /^Request funds$/i)));
  await sleep(4000);
  await dump(p, 'request funds dialog');

  await p.evaluate(() => {
    const el = document.getElementById('funding-note');
    const set = Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype, 'value').set;
    set.call(el, 'Terms as discussed: $120,000 for 7.2% on a $1.4M pre-money. '
      + 'The first twenty arrays are already scheduled with the supplier.');
    el.dispatchEvent(new Event('input', { bubbles: true }));
    el.dispatchEvent(new Event('change', { bubbles: true }));
  });
  await sleep(900);
  console.log('sent: ' + JSON.stringify(await clickText(p, /^Send request$/i, '[role="dialog"]')));
  await sleep(9000);

  await p.goto(ORIGIN + ROOM, { waitUntil: 'networkidle2' });
  await settle(p);
  await dump(p, 'deal room after the funding request');
  await b.close();
})();
