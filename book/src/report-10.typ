...
#show: report.with(number: 10, name: "Payments")

= Introduction

Report 9 left an approved commitment: a specific amount, on a specific venture,
...
Report 9 owns `Investment`. This report owns the three objects beneath it, and
...
#delivered[
  *A refund needs no special case in the arithmetic.* A reversed transaction
  ...
  Subtraction is a consequence of the predicate in Report 9 rather than a second
...
#note[
  *This capture required a whole journey.* A checkout session cannot be
  ...
  That is the argument of Report 9 restated as a practical fact: *an approved
...
exception to the derive-don't-store rule that governs Report 9: the fee schedule
...
*What this enables.* Report 7 covers the conversation that surrounds all of this
