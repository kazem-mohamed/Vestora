...
#show: report.with(number: 7, name: "Messaging & Notifications")

= Introduction

Report 10 covered the moment money moves. This report covers the conversation
...
documents and the commitment it concerns — is Report 8.
...
#figure(
  table(
    ...
    [Image attachments], [Validated by the same upload controls as any other
      image, from Report 4, and served through an authorised route rather than a
      public path.],
    [Message deletion], [A sender can remove their own message. Nobody can
    ...
    [Actionable notifications], [A commitment can be accepted or declined from
      the notification itself.],
    [Background fan-out], [Notification delivery runs off the request path.],
  ),
  caption: [Features delivered in this part. The deal room, which composes this
    conversation beside a relationship's documents and terms, is Report 8.],
)
...
#figure(
  table(
    ...
    [`POST`], [`api/messages/send`], [Send a message; persisted, then pushed.],
    [`POST`], [`api/messages/send-attachment`], [Send with an image, validated on
      its bytes.],
    [`GET`], [`api/messages/attachment/{messageId}`], [Fetch an attachment
      through an authorised route rather than from a public path.],
    [`GET`], [`api/messages/conversation/{senderId}/{receiverId}`],
      [One thread's history.],
    [`GET`], [`api/messages/conversations/{userId}`], [Every thread, with unread
      counts.],
    [`POST`], [`api/messages/read/{partnerId}`], [Mark a thread read —
      server-side.],
    [`GET`], [`api/messages/unread-count`], [The badge figure.],
    [`GET`], [`api/messages/message/{id}`], [One message.],
    [`DELETE`], [`api/messages/message/{id}`], [Delete one's own message.],
    [`GET`], [`api/notification/{userId}/notifications`], [The user's
      notifications.],
    [`POST`], [`api/notification/notifications/{id}/mark-as-read`], [Mark one
      read.],
    [`POST`], [`api/notification/{id}/approve-support`], [Accept a commitment
      *from the notification that announced it*.],
    [`POST`], [`api/notification/{id}/reject-support`], [Decline it.],
...
*The attachment has its own authorised route.* An image sent in a private
conversation is not written to a path that anyone holding the URL can fetch; it
is served by an endpoint that checks the caller is party to the thread. This is
the same reasoning as the document downloads in Report 8, applied to the
lighter-weight case.

*The last two rows are not messaging at all.* A founder accepts or declines a
commitment from the notification that told them about it, rather than from an
investment route. The design follows the founder's real path — the notification
is how they learn the request exists, so it is where the decision is offered. The
cost is that a money-adjacent rule lives on a controller whose name does not
suggest it, which is a discovery problem for a maintainer rather than for a user.

The hub deserves the final row. It exposes exactly three methods — `SendMessage`,
`Typing` and `GetPresence` — and shares the API's authentication pipeline
...
#challenge("Notification fan-out tied a business action to its side effects")[
  ...
  *The cost, stated.* The queue is process-local, so work enqueued and not yet
  ...
  than assumed away. It would not be acceptable for anything in Report 10.
]
...
*What this enables.* Report 8 takes this conversation, scopes it to a single
